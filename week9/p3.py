...
def preprocess_digit(roi):
    ...
    roi_eroded = cv2.erode(roi, kernel, iterations=3)

    pad = 3
    roi_padded = cv2.copyMakeBorder(roi_eroded, pad, pad, pad, pad, cv2.BORDER_CONSTANT, value=0)
...
for i, d in enumerate(digit_list):
    ...
    _, result, _, dist = knn.findNearest(sample, k=3)
